...
aspirinas = json.loads(aspirinas_raw)['results']


# Iteramos con un bucle for sobre los distintos medicamentos contenedores de aspirina obtenidos tras nuestra busqueda
for asp in range(len(aspirinas)):
    # Para evitar el error usamos try-except en caso de que el fabricante del medicamento no aparezca como dato
    try:
        print('El medicamento cuyo identificador es: ', aspirinas[asp]['id'], 'es fabricado por: ',
              aspirinas[asp]['openfda']['manufacturer_name'][0], '.')
    except KeyError:
        print('El medicamento cuyo identificador es: ', aspirinas[asp]['id'], 'no tiene un fabricante reconocible.')
